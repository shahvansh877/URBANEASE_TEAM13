...
router.post("/", protect, isUser, async (req, res) => {
  try {
    ...
    req.app.get("realtime")?.emitBookingEvent(populated, "booking:created");
...
router.put("/:id/status", protect, isVerifiedProvider, async (req, res) => {
  try {
    ...
    req.app.get("realtime")?.emitBookingEvent(populated, "booking:status-updated");

    return res.status(200).json({ success: true, message: "Booking status updated", booking: populated });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
});
...
router.put("/:id/payment", protect, isUser, async (req, res) => {
  try {
    ...
    req.app.get("realtime")?.emitBookingEvent(populated, "booking:payment-updated");

    return res.status(200).json({ success: true, message: "Payment updated", booking: populated });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
});
...
router.put("/:id/review", protect, isUser, async (req, res) => {
  try {
    ...
    req.app.get("realtime")?.emitBookingEvent(updatedBooking, "booking:reviewed");
...
router.delete("/:id", protect, async (req, res) => {
  try {
    ...
    req.app.get("realtime")?.emitBookingEvent(booking, "booking:deleted");
